...
int main()
{
    ...
    for(int j=0; j<n;j++){
        ...
        for(int i=0; i<s;i++){
            ...
            String img_name = "image_" + string("level_")+to_string(j)+ string("_diff_level_")+to_string(cnt);
            ...
            if(i==(s-1) and j==(n-1)){
                ...
                cnt +=1;
                sigma = k*sigma;
                gk_2d = Gaussianfilter2D(filtersize,filtersize,sigma);
                padded2d_prev = twoDpadding(normalized_prev,filtersize);
                twoDconvOutput = twoDconv(padded2d_prev,gk_2d);
                normalized = twoDnormalize(twoDconvOutput);
                diff =  difference(normalized_prev,normalized);
                normalized_final = twoDnormalize(diff);
                convimg = matrix2img(normalized_final);
                img_name = "image_" + string("level_")+to_string(j)+ string("_diff_level_")+to_string(cnt);
                im_save_name = img_name + string(".png");
                imwrite(im_save_name,convimg);
            }
